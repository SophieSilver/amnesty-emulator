...
pub fn dex<M: Memory>(cpu: &mut Cpu, memory: &mut M) -> ControlFlow<()> {
    templates::implied(cpu, memory, |cpu| {
        ...
        set_register_with_flags(&mut cpu.x_index, new_value, &mut cpu.flags)
    })
}
...
pub fn dey<M: Memory>(cpu: &mut Cpu, memory: &mut M) -> ControlFlow<()> {
    templates::implied(cpu, memory, |cpu| {
        ...
        set_register_with_flags(&mut cpu.y_index, new_value, &mut cpu.flags)
    })
}
...
pub fn tax<M: Memory>(cpu: &mut Cpu, memory: &mut M) -> ControlFlow<()> {
    templates::implied(cpu, memory, |cpu| {
        set_register_with_flags(&mut cpu.x_index, cpu.accumulator, &mut cpu.flags);
    })
}

pub fn tay<M: Memory>(cpu: &mut Cpu, memory: &mut M) -> ControlFlow<()> {
    templates::implied(cpu, memory, |cpu| {
        set_register_with_flags(&mut cpu.y_index, cpu.accumulator, &mut cpu.flags);
    })
}

pub fn tsx<M: Memory>(cpu: &mut Cpu, memory: &mut M) -> ControlFlow<()> {
    templates::implied(cpu, memory, |cpu| {
        set_register_with_flags(&mut cpu.x_index, cpu.stack_ptr, &mut cpu.flags);
    })
}

pub fn txa<M: Memory>(cpu: &mut Cpu, memory: &mut M) -> ControlFlow<()> {
    templates::implied(cpu, memory, |cpu| {
        set_register_with_flags(&mut cpu.accumulator, cpu.x_index, &mut cpu.flags);
    })
}
...
pub fn tya<M: Memory>(cpu: &mut Cpu, memory: &mut M) -> ControlFlow<()> {
    templates::implied(cpu, memory, |cpu| {
        set_register_with_flags(&mut cpu.accumulator, cpu.y_index, &mut cpu.flags);
    })
}
